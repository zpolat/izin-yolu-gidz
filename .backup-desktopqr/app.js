...
(function installHint() {
  ...
  if (standalone) return;                                    // zaten kurulu
  try { if (localStorage.getItem('installHintDismissed') === '1') return; } catch (e) {}

  const ua = navigator.userAgent || '';
  const isIOS = /iphone|ipad|ipod/i.test(ua);
  const addBtn = document.getElementById('installAdd');
  const closeBtn = document.getElementById('installClose');
  const textEl = bar.querySelector('.ib-text');
  let deferred = null;
  ...
  // Android / Chrome: platform kendi kurulum kutusunu verir
  ...
  addBtn.addEventListener('click', async () => {
    if (deferred) {
      ...
      // iOS otomatik kuramaz → elle talimat göster
      textEl.innerHTML = 'Alttaki <b>Paylaş</b> ⬆️ → <b>“Ana Ekrana Ekle”</b>';
      addBtn.hidden = true;
    } else {
      dismiss();
    }
  ...
  // iOS'ta beforeinstallprompt yok; barı kısa gecikmeyle biz gösteririz
